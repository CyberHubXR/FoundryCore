...
namespace CyberHub.Brane.Setup
{
    ...
    /// <summary>
    /// Represents a task that is required or suggested to be completed to set up a Brane project
    /// </summary>
    ...
    public class BraneSetupWizard : EditorWindow
    {
        [MenuItem("Brane/Setup Wizard", false, 1)]
        public static void OpenWindow()
        {
            var window = GetWindow<BraneSetupWizard>(false, "Setup Wizard", true);
            window.Repaint();
        }
        ...
        private static void CheckForTasks()
        {
            ...
            foreach (var module in modulesTypes)
            {
                ...
                {
                    Debug.LogError($"BraneSetupWizard was unable to load {module.Name} since it does not have a default constructor!");
                    continue;
                }
        ...
        public void CreateGUI()
        {
            ...
            if (!pendingTasks)
            {
                VisualElement noItems = new Label("Nothing left to do, you're all set! Thanks for using Brane!");
                root.Add(noItems);
            }
        }
    }
