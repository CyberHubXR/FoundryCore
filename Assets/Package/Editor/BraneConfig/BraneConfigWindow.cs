...
{
    public class BraneConfigWindow : EditorWindow
    {
        [MenuItem("Brane/Config Manager", false, 1)]
        public static void OpenWindow()
        {
            var window = GetWindow<BraneConfigWindow>(false, "Brane Config Manager", true);
            window.Repaint();
        }
        ...
        private static void CheckIfProjectValid()
        {
            ...
            var config = BraneAppConfig.GetAsset();
        ...
        private static void UpdateModuleDefinitions()
        {
            ...
            var modules = BraneAppConfig.GetAsset().modules ?? new BraneModuleConfig[0];
            bool changed = false;
            ...
            if (changed)
            {
                BraneAppConfig.GetAsset().modules = moduleConfigs;
                EditorUtility.SetDirty(BraneAppConfig.GetAsset());
            }
        }
        ...
        private void CreateGUI()
        {
            ...
            var serviceProviders = new Dictionary<Type, List<KeyValuePair<ProvidedService, BraneModuleConfig>>>();
            ...
            foreach (var module in _moduleDefinitions)
            {
                ...
                {
                    List<KeyValuePair<ProvidedService, BraneModuleConfig>> providerList;
            ...
            foreach (var service in _serviceDefinitions)
            {
                ...
                var providerLocations = new List<BraneModuleConfig>();
                if (!required)
                {
